...
vim.api.nvim_create_user_command("CopyRelPathWithLineRange", function()
  local path = relpath()
  local line_start, col_start = vim.fn.line("'<"), vim.fn.col("'<")
  local line_end, col_end = vim.fn.line("'>"), vim.fn.col("'>")
  local text = string.format("%s:%d-%d", path, line_start, line_end)
  vim.fn.setreg("+", text)
  notify(text, "info", { title = "Copied relative path with line range" })
end, { range = true })
...
keymap("v", "<leader>cp", "<cmd>CopyRelPathWithLineRange<CR>", opts)
